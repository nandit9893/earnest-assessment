...
import { createTaskFailure, createTaskStart, createTaskSuccess } from "@/Redux/User/UserSlice";
import { addTask } from "@/Utils/API/task";
...
const AddTask: React.FC<AddTaskProps> = ({ closeModal, showAddTask }) => {
  ...
  const submitForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await addTask(user?.accessToken, payload);
    if(result?.success) {
          ...
          <div className="absolute left-8 right-0 top-0 bottom-0 bg-linear-to-br from-[#FAF3E1] to-[#d4b595] shadow-inner">
            <div className="flex flex-col w-full h-full">
              <div className="flex justify-between w-full items-center px-5 pt-3 pb-1">
                <p className="text-[18px] leading-7 text-black font-normal">New Task</p>
